...
public class HumanHeroUnit : UnitBase
{
    public int index = -1;
    public Animator heroAnimator;
    public GameObject heroGunPrefab;
    ...
    void HeroFire()
    {
        for (int i = 0; i < GameplayController.instance.enemyArchers.Length; i++)
        {
            ...
            if (heroObjects == null)
                return;
            GameObject go = Instantiate(heroGunPrefab, heroObjects.transform.GetChild(1));
            go.transform.LookAt(GameplayController.instance.enemyArchers[i].transform.GetChild(0));
            go.GetComponent<Ammo>().prefab = heroBulletPrefab;
        }
    }
