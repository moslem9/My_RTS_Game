...
public class HumanBarrackBuilding : BuildingBase
{
    ...
    public void HumanArcherAndFootman(bool asButton, Transform[] spawnPoints) {
        ...
        go.GetComponentInChildren<TMP_Text>().text = "Footman";
        ...
        GameObject go2 = Instantiate(rangePrefab
          , spawnPoints[UnityEngine.Random.Range(0, spawnPoints.Length)].position, Quaternion.identity);
        go2.GetComponentInChildren<TMP_Text>().text = "Archer";
        go2.GetComponent<HumanRangeUnit>().enabled = true;
        go2.GetComponent<HumanRangeUnit>().rangeObjects.SetActive(true);
    }
